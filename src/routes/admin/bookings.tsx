  ...
  return c.html(
    <Layout title={`${title} - Admin`}>
      <div class="max-w-5xl mx-auto p-6">
        ...
        {/* Filters */}
        <form method="get" action={baseUrl} class="bg-white rounded-xl border border-slate-200 p-4 mb-6">
          <div class="flex flex-wrap gap-3 items-end">
            {!eventId && (
              <div>
                <label class="block text-xs font-medium text-slate-500 mb-1">Event</label>
                <select name="event" class="border border-slate-300 rounded-md px-3 py-1.5 text-sm" onchange="this.form.action='/admin/events/'+this.value+'/bookings'; if(!this.value) this.form.action='/admin/bookings'; this.form.submit()">
            ...
            <div>
              ...
              <input type="text" name="search" value={search || ""} placeholder="Name or email" class="border border-slate-300 rounded-md px-3 py-1.5 text-sm w-48" />
            </div>
            <button type="submit" class="bg-slate-100 hover:bg-slate-200 px-4 py-1.5 rounded-md text-sm font-medium text-slate-600">
              Filter
            </button>
            <a href={baseUrl} class="text-slate-400 text-sm hover:text-slate-500 py-1.5">Clear</a>
          </div>
        </form>

        <div class="flex items-center justify-between mb-3">
          <span class="text-sm text-slate-500">{result.total} bookings found</span>
          {eventId && (
            <form method="post" action={`/admin/events/${eventId}/bookings/export.csv`}>
              <input type="hidden" name="status" value={status || ""} />
              <input type="hidden" name="search" value={search || ""} />
              <button type="submit" class="border border-slate-300 text-slate-600 px-3 py-1.5 rounded-md text-sm hover:bg-slate-50">
                Export CSV
              </button>
            </form>
          )}
        ...
        {result.rows.length === 0 ? (
          <p class="text-slate-500">No bookings match your filters.</p>
        ) : (
          <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <table class="w-full text-sm">
              <thead class="bg-slate-50 border-b border-slate-200">
                <tr>
                  {!eventId && <th class="text-left px-4 py-3 font-medium text-slate-500">Event</th>}
                  <th class="text-left px-4 py-3 font-medium text-slate-500">Invitee</th>
                  <th class="text-left px-4 py-3 font-medium text-slate-500">Date & Time</th>
                  <th class="text-left px-4 py-3 font-medium text-slate-500">Status</th>
                  <th class="text-left px-4 py-3 font-medium text-slate-500">Actions</th>
                </tr>
              </thead>
              <tbody>
                {result.rows.map((b) => (
                  <tr class="border-b border-slate-100">
                    ...
                    <td class="px-4 py-3 space-x-2">
                      <a href={bookingUrl(b.event_type_id, b.id)} class="text-indigo-600 hover:underline text-xs">
                        View
                      </a>
                      {b.status === "confirmed" && (
                        <button
        ...
        {result.rows.filter((b) => b.status === "confirmed").map((b) => (
          <dialog id={`cancel-modal-${b.id}`} class="rounded-lg shadow-xl p-0 backdrop:bg-black/30">
            <form method="post" action={cancelUrl(b.event_type_id, b.id)} class="p-6 w-80">
              ...
              <textarea name="reason" required rows={3} placeholder="Cancel reason..." class="w-full border border-slate-300 rounded-md px-3 py-2 text-sm mb-4" />
              <div class="flex gap-2 justify-end">
                <button type="button" class="px-3 py-1.5 text-sm text-slate-500" onclick={`document.getElementById('cancel-modal-${b.id}').close()`}>Back</button>
        ...
        {result.totalPages > 1 && (
          <div class="flex items-center justify-center gap-2 mt-6">
            {result.page > 1 && (
              <a href={buildUrl({ page: result.page - 1 })} class="px-3 py-1 border border-slate-300 rounded text-sm hover:bg-slate-50">
                Prev
              </a>
            )}
      ...
      <div class="max-w-2xl mx-auto p-6">
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
          ...
          <div class="border-b border-slate-100 pb-4">
            <h2 class="text-sm font-medium text-slate-500 mb-2">Event</h2>
            <p class="font-semibold text-slate-900">{b.event_name}</p>
            <p class="text-sm text-slate-500">{(b as any).host_name} &middot; {(b as any).duration_minutes} min</p>
          </div>

          <div class="border-b border-slate-100 pb-4">
            <h2 class="text-sm font-medium text-slate-500 mb-2">When</h2>
            <p class="text-slate-900">{dateLabel}</p>
            <p class="text-slate-900">{startTime} - {endTime}</p>
            <p class="text-sm text-slate-400 mt-1">{b.timezone}</p>
          </div>

          <div class="border-b border-slate-100 pb-4">
            <h2 class="text-sm font-medium text-slate-500 mb-2">Invitee</h2>
            <p class="text-slate-900">{b.invitee_name}</p>
            <p class="text-sm text-slate-500">{b.invitee_email}</p>
          </div>

          {b.meeting_url && (
            <div class="border-b border-slate-100 pb-4">
              <h2 class="text-sm font-medium text-slate-500 mb-2">Meeting</h2>
              <a href={b.meeting_url} target="_blank" class="text-indigo-600 hover:underline text-sm">
                {b.meeting_url}
              </a>
              {b.meeting_code && (
                <p class="text-sm text-slate-400 mt-1">Code: {b.meeting_code}</p>
              )}
            </div>
          )}
          ...
          {customFields.length > 0 && (
            <div class="border-b border-slate-100 pb-4">
              <h2 class="text-sm font-medium text-slate-500 mb-2">Additional Info</h2>
              <div class="space-y-1">
                {customFields.map((f) => (
                  <div class="flex">
                    <span class="text-sm text-slate-500 w-32">{f.label}</span>
                    <span class="text-sm text-slate-900">{customData[f.key] || "-"}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {b.notes && (
            <div class="border-b border-slate-100 pb-4">
              <h2 class="text-sm font-medium text-slate-500 mb-2">Notes</h2>
              <p class="text-sm text-slate-900 whitespace-pre-wrap">{b.notes}</p>
            </div>
          )}
          ...
          {b.cancel_reason && (
            <div class="border-b border-slate-100 pb-4">
              ...
              <p class="text-sm text-slate-900 whitespace-pre-wrap">{b.cancel_reason}</p>
            </div>
          )}

          <div>
            <h2 class="text-sm font-medium text-slate-500 mb-2">Meta</h2>
        ...
        <dialog id="cancel-modal-detail" class="rounded-lg shadow-xl p-0 backdrop:bg-black/30">
          <form method="post" action={cancelUrl(eventId, b.id)} class="p-6 w-96">
            <h3 class="font-semibold text-slate-900 mb-3">Cancel Booking</h3>
            <p class="text-sm text-slate-500 mb-3">{b.event_name} &middot; {b.invitee_name}</p>
            <textarea name="reason" required rows={3} placeholder="Cancel reason..." class="w-full border border-slate-300 rounded-md px-3 py-2 text-sm mb-4" />
            <div class="flex gap-2 justify-end">
              <button type="button" class="px-3 py-1.5 text-sm text-slate-500" onclick="document.getElementById('cancel-modal-detail').close()">Back</button>
              <button type="submit" class="bg-red-600 text-white px-4 py-1.5 rounded-md text-sm font-medium hover:bg-red-700">Confirm</button>
            </div>
          </form>
        </dialog>
